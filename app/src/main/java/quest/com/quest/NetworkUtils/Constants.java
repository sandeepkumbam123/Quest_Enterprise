package quest.com.quest.NetworkUtils;

/**
 * Created by kumbh on 19-03-2017.
 */

public class Constants {

    public static final String API = "api/";
    public static final String LOCAL_SERVER_IP ="http://172.20.10.4/";
    public static final String APPLICATION_BASE_URL = LOCAL_SERVER_IP+"Quest/" +API;

    public static final String QUEST_URL = LOCAL_SERVER_IP+"Quest/";
    public static final String LOGIN_URL ="login";
    public static final String FORGOT_PASSWORD ="";


    public static final String ENABLE_EXAM ="exam/enable";
    public static final String DISABLE_EXAM ="exam/disable";
    public static final String START_EXAM ="exam/start-exam";
    public static final String SUBMIT_EXAM ="exam/submit-exam";

    public static final String EXAMS ="exams";

    public static final String PAST_EXAMS_RESULT = "exam/past-exam-result";
    public static final String PAST_EXAMS ="exam/past-exams";
    public static final String STUDENT_EXAMS_RESULT ="one-student-past-exams";

    public static final String IMAGE_URL = QUEST_URL + "public/";

    public static final String EXAM_ID = "exam/get-exam-id";







}
